...
RUN tar -zxvf leptonica-1.77.0.tar.gz
WORKDIR leptonica-1.77.0/
RUN ./autogen.sh
RUN ./configure
RUN make
RUN make install
...
# build tesseract
RUN wget -O tesseract-ocr-3.02.02.tar.gz https://github.com/tesseract-ocr/tesseract/archive/3.02.02.tar.gz
...
RUN wget https://cfhcable.dl.sourceforge.net/project/tesseract-ocr-alt/tesseract-ocr-3.02.eng.tar.gz
